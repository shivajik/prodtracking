...
export async function seedCropsAndVarieties() {
  console.log("Starting to seed crops and varieties...");
  
  try {
    for (const [cropName, varieties] of Object.entries(cropMarketData)) {
      // Create crop
      console.log(`Creating crop: ${cropName}`);
      const crop = await apiRequest("/api/crops", {
        method: "POST",
        body: JSON.stringify({ name: cropName }),
      }) as any;
      
      console.log(`Created crop: ${cropName} with ID: ${crop.id}`);
      
      // Create varieties for this crop
      for (const varietyCode of varieties) {
        console.log(`Creating variety: ${varietyCode} for crop: ${cropName}`);
        await apiRequest("/api/varieties", {
          method: "POST", 
          body: JSON.stringify({ code: varietyCode, cropId: crop.id }),
        });
      }
      
      console.log(`✅ Completed crop: ${cropName} with ${varieties.length} varieties`);
    }
    
    console.log("🌱 Successfully seeded all crops and varieties!");
    return true;
  } catch (error) {
